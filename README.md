*README на русском €зыке*: [[README.ru.md](README.ru.md)]
...
To add a new block (record) to the chain, a computationally expensive operation must be performed Ч
finding a hash that meets specific conditions Ч the hash must contain a certain number of consecutive zero bits.

Verifying an existing block, on the other hand, requires very little computational effort Ч
...
the result of computing the blockТs SHA-256 hash meets the defined difficulty target.
